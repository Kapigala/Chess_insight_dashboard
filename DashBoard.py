...
def df_init(rok,miesiac,tempo):
    global gracze
    dane=pd.read_csv('chess_{}_rating_{}.csv'.format(rok,tempo),names=h)
...
def bar_chart(lista=['POL','RUS','GER','ESP','IND']):
    ...
    for i in df_1['federation'].unique():
        for title_1 in ['WCM','CM','WFM','FM','WIM','IM','WGM','GM']:
            df=df.append({'federation':i,'title':title_1,'count':df_1[(df_1['federation']==i) & (df_1['title']==title_1)].shape[0]},ignore_index=True)
...
def mapa(tryb='GM_COUNT',rok='2020',miesiac='Dec',tempo='standard'):
    ...
    if tryb =='AVG_ELO' or tryb=='MAX_ELO':
        ...
        for i in df_base['federation'].unique():
            for title_1 in ['WCM','CM','WFM','FM','WIM','IM','WGM','GM']:
                df=df.append({'federation':i,'title':title_1,'count':df_base[(df_base['federation']==i) & (df_base['title']==title_1)].shape[0]},ignore_index=True)
...
def update_liste(v1,v2,v3):
    ...
    return '{}  {} lat,  {},  ranking - {},  {}'.format(ll['title'].values[0],ll['name'].values[0],2020-ll['yob'].values[0],ll['Dec'].values[0],ll['federation'].values[0])
...
def render_content(tab):
    if tab == 'tab-0-example-graph':
        return html.Div([
            ...
            {'fontSize': 20}),
            dcc.Dropdown(id='suwak_2',options=[{'label': i, 'value': i} for i in ['standard','rapid','blitz']],value='standard'),
            dcc.Dropdown(id='suwak_8',options=[{'label': i, 'value': i} for i in ['Men','Women','All']],value='All',placeholder='Select gender'),
        ...
        return html.Div([
            ...
            dcc.Dropdown(id='suwak_1',options=[{'label': i, 'value': i} for i in ['2015','2016','2017','2018','2019','2020']], value='2020', placeholder='Select year'),
            dcc.Dropdown(id='suwak_2',options=[{'label': i, 'value': i} for i in ['standard','rapid','blitz']],value='standard'),
        ...
        return html.Div([
            ...
            dcc.Graph(
                id='graph3',
                figure=hist_fig),
            dcc.Dropdown(id='suwak_2',options=[{'label': i, 'value': i} for i in ['standard','rapid','blitz']],value='standard',placeholder='Tempo'),
            dcc.Slider(id='slide_2',
                       min=2000,
                       max=2800,
                       step=100,
                       value=2000,
                       marks={2000:'2000',2100:'2100',2200:'2200',2300:'2300',2400:'2400',2500:'2500',2600:'2600',2700:'2700',2800:'2800'}
        ...
        return html.Div([
            ...
            dcc.Dropdown(id='suwak_7',options=[{'label': i, 'value': i} for i in lata],value='1990-2000',placeholder='Select years')
        ])
        ...
        return html.Div([
            ...
            dcc.Dropdown(id='suwak_4',options=[{'label': i, 'value': i} for i in np.sort(gracze['federation'].unique())],value=['POL','RUS','GER','ESP','IND'],placeholder='Select country',multi=True)
        ])
...
app.run_server(debug=False,dev_tools_ui=False,dev_tools_props_check=False, port=os.getenv('PORT', '8050'), host="0.0.0.0")
