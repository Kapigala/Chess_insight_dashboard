...
def hist_b(lata='1990-2000'):
    ...
    if lata == 'All':
        ...
        fig = px.histogram(gracze[(gracze['yob']<int(lata[5:9])) & (gracze['yob']>int(lata[0:4]))]['federation'],
                           height=800,
                           title='Liczba utytułowanych zawodników urodzonych w latach {}'.format(lata)).update_xaxes(categoryorder='total descending')
        return fig
...
app.layout = html.Div(children=[
    html.Div([
        ...
        dcc.Dropdown(id='suwak_5',options=[{'label': i, 'value': i} for i in ['GM','WGM','CM','WCM','IM','WIM','FM','WFM']],value='GM',placeholder='Select title'),
        dcc.Dropdown(id='suwak_6',options=[{'label': i, 'value': i} for i in ['AVG_ELO','MAX_ELO','GM_COUNT','IM_COUNT','WGM_COUNT','WIM_COUNT']],value='AVG_ELO',placeholder='Select feature'),
